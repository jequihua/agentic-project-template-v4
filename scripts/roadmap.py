...
SLICE = {
    ...
    'allowed_prefixes', 'focused', 'verification', 'risk', 'kind', 'memory_pages', 'notes',
}
...
def validate(data: dict) -> tuple[list[str], list[str]]:
    ...
    if memory is not None and _map(memory, MEMORY, 'memory', errors):
        ...
        memory_root = (
            ...
            if isinstance(memory.get('root'), str) else None
        )
...
def load(root: Path=c.ROOT) -> dict:
    ...
    out['review'] = {
        key: [seats] if isinstance(seats, str) else seats
        for key, seats in out['review'].items()
    }
    return out
...
def render_markdown(data):
    ...
    for milestone in data['milestones']:
        lines += [
            ...
            f"Status: {milestone['status']}", f"Risk: {milestone['risk']}",
