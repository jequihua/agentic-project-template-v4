...
import subprocess as sp, sys, tempfile as tf, tomllib as tl
...
def reject_symlinks(root, rel, label):
    cursor = root.resolve()
    for part in P(safe_rel(rel)).parts:
        cursor /= part
...
def load_manifest(path):
    try:
        data = tl.loads(path.read_text(encoding='utf-8'))
    except (OSError, UnicodeError, tl.TOMLDecodeError) as exc:
...
def git_exe(root):
    ...
    if not local.exists():
        return 'git'
    try:
        data = tl.loads(local.read_text(encoding='utf-8'))
    except (OSError, UnicodeError, tl.TOMLDecodeError) as exc:
...
def is_sensitive(rel):
    low, parts = (rel.lower(), P(rel.lower()).parts)
...
def plan(manifest, source, target, allow_sensitive=False):
    ...
    for row in manifest['directories']:
        ...
        managed.append(base)
        walked = walk_source(
            source, row['source'], rules, row.get('exclude', []), allow_sensitive
        )
...
def divergences(entries, state):
    ...
    for rel, old in recorded.items():
        if not any((x['rel'] == rel for x in entries)):
            if entries:
                depth = len(P(entries[0]['rel']).parts) - 1
                path = entries[0]['target'].parents[depth] / rel
            else:
                path = None
            changed = path and path.exists() and (
                not path.is_file() or path.is_symlink() or digest(path) != old
            )
            if changed:
                out.append(rel)
...
def write_copy(src, dst):
    ...
    fd, name = tf.mkstemp(prefix=f'.{dst.name}.', dir=dst.parent)
    os.close(fd)
    try:
...
def write_state(target, value):
    ...
    path = target / STATE
    fd, name = tf.mkstemp(prefix='.front-state.', dir=target)
    try:
